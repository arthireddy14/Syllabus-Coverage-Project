...
class ProgressService:
    ...
    def mark_topic_progress(self, sid: int, tpid: int, lastupdate: str = None):
        # Check if progress exists
        existing = self.dao.get_progress_by_student(sid)
        for p in existing:
            if p['tpid'] == tpid:
                return self.dao.update_progress(sid, tpid, lastupdate)
        # If not exists, add new
        return self.dao.add_progress(sid, tpid, lastupdate)
